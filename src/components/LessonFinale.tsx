...
function OrderRound({question,correct,onChoose}:{question:Question;correct:boolean;onChoose:(index:number)=>void}) {
  const tokens=question.options[question.answer].split('→').map((token)=>token.trim());
  const [pool,setPool]=useState(()=>tokens.map((token,index)=>({token,key:`${index}-${token}`})).reverse());
  const [line,setLine]=useState<Array<{token:string;key:string}>>([]);
  const place=(item:{token:string;key:string})=>{if(correct)return;setPool((items)=>items.filter((entry)=>entry.key!==item.key));setLine((items)=>[...items,item]);};
  const reset=()=>{setPool(tokens.map((token,index)=>({token,key:`${index}-${token}`})).reverse());setLine([]);};
  const verify=()=>{const built=line.map((item)=>item.token).join(' → ');const expected=tokens.join(' → ');onChoose(built===expected?question.answer:(question.answer===0?1:0));};
  return <div className="finale-order-lab"><div className="order-line">{tokens.map((_,index)=><span key={index} className={line[index]?'filled':''}>{line[index]?.token??`${index+1}. adım`}{index<tokens.length-1&&<i>→</i>}</span>)}</div><div className="order-pool">{pool.map((item)=><button key={item.key} type="button" onClick={()=>place(item)}>{item.token}</button>)}</div><footer><button type="button" onClick={reset} disabled={correct||line.length===0}>Sıfırla</button><button type="button" className="button button-primary" disabled={correct||line.length!==tokens.length} onClick={verify}>Akışı çalıştır<Icon name="spark" /></button></footer></div>;
}

function FinaleInteraction({question,selected,correct,onChoose}:{question:Question;selected:number|null;correct:boolean;onChoose:(index:number)=>void}) {
  const [cursor,setCursor]=useState(0);
  const [armed,setArmed]=useState<number|null>(null);
  if(question.type==='sıralama')return <OrderRound question={question} correct={correct} onChoose={onChoose}/>;
  if(question.type==='kavram')return <div className="finale-concept-radar"><div className="concept-radar-core"><i/><i/><span>?</span><small>KAVRAM SİNYALİ</small></div><div>{question.options.map((option,index)=><button key={option} type="button" disabled={correct} onClick={()=>onChoose(index)} className={selected===index?(index===question.answer?'correct':'incorrect'):''}><b>{String(index+1).padStart(2,'0')}</b><span>{option}</span><i/></button>)}</div></div>;
  if(question.type==='hata')return <div className="finale-fault-map"><div className="fault-monitor"><small>SİSTEM TARAMASI</small><i/><strong>İlk kırık düğüm nerede?</strong></div><div>{question.options.map((option,index)=><button key={option} type="button" disabled={correct} onClick={()=>onChoose(index)} className={selected===index?(index===question.answer?'correct':'incorrect'):''}><b>{selected===index?'!':'?'}</b><span>{option}</span><small>{selected===index?'Tarandı':'Düğümü tara'}</small></button>)}</div></div>;
  if(question.type==='senaryo')return <div className="finale-scenario-sim"><div className="scenario-stage"><div className={selected===null?'scenario-avatar':correct?'scenario-avatar correct':'scenario-avatar incorrect'}><i/><span>{selected===null?'BEKLİYOR':correct?'DENGEDE':'RİSKLİ'}</span></div><p>{question.options[cursor]}</p></div><aside><label>Olası sonucu değiştir<input type="range" min="0" max={question.options.length-1} value={cursor} disabled={correct} onChange={(event)=>setCursor(Number(event.target.value))}/></label><div>{question.options.map((_,index)=><i key={index} className={cursor===index?'active':''}/>)}</div><button type="button" className="button button-primary" disabled={correct} onClick={()=>onChoose(cursor)}>Senaryoyu oynat<Icon name="spark" /></button></aside></div>;
  if(question.type==='transfer')return <div className="finale-transfer-board"><div className="transfer-source"><small>ÖĞRENİLEN FİKİR</small><b>◆</b><strong>Yeni duruma taşı</strong></div><div className="transfer-lines" aria-hidden="true">{question.options.map((_,index)=><i key={index} className={selected===index?(index===question.answer?'correct':'incorrect'):''}/>)}</div><div className="transfer-targets">{question.options.map((option,index)=><button key={option} type="button" disabled={correct} onClick={()=>onChoose(index)} className={selected===index?(index===question.answer?'correct':'incorrect'):''}><span>{index+1}</span><strong>{option}</strong><small>Kabloyu buraya bağla</small></button>)}</div></div>;
  return <div className="finale-boss-console"><div className="boss-display"><small>SON KARAR TERMİNALİ</small><strong>{armed===null?'Kanıt paketini seç':question.options[armed]}</strong><div><i className={armed!==null?'active':''}/><i className={selected!==null?'active':''}/><i className={correct?'active':''}/></div><p>Seçim → doğrulama → sonuç</p></div><div className="boss-evidence">{question.options.map((option,index)=><button key={option} type="button" disabled={correct} className={armed===index?'armed':''} onClick={()=>setArmed(index)}><b>{index+1}</b><span>{option}</span></button>)}<button className="button button-primary" type="button" disabled={armed===null||correct} onClick={()=>armed!==null&&onChoose(armed)}>Kararı doğrula<Icon name="check" /></button></div></div>;
}
...
export function LessonFinale({lesson,locale,onComplete,onReview}:{lesson:Lesson;locale:Locale;onComplete:()=>void;onReview?:(stepId:string)=>void}) {
  ...
  if(!started)return <section className="finale-launch"><div className="finale-orbit" aria-hidden="true"><i/><i/><i/><span><Icon name="spark" /></span></div><div><p className="section-kicker">DERS SONU · EN YÜKSEK XP</p><h2>{pack.title}</h2><p>{pack.subtitle} İlk seçimin ustalık puanına girer. Yanlış bir seçim yaptığında puan kaybetmezsin; açıklamayı okuyup soruyu yeniden düşünebilirsin.</p><ul><li><b>12</b><span>farklı tur</span></li><li><b>%70</b><span>hedef başarı</span></li><li><b>{lesson.steps.at(-1)?.xp}</b><span>kazanılabilir XP</span></li></ul><button className="button button-primary finale-start" type="button" onClick={()=>setStarted(true)}>Ders sonu uygulamasına başla<Icon name="arrow-right" /></button></div></section>;
  ...
  return <section className={`finale-arena ${question.type}`}><header><div><span>{labels[question.type]}</span><strong>{index+1} / {pack.questions.length}</strong></div><div className="finale-progress"><i style={{width:`${((index+1)/pack.questions.length)*100}%`}} /></div><p><Icon name="spark" /> İlk deneme: {firstAttempts.filter(Boolean).length} doğru</p></header><main><p className="section-kicker">{pack.title}</p><h3>{question.prompt}</h3><FinaleInteraction key={`${index}-${question.prompt}`} question={question} selected={selected} correct={correct} onChoose={choose}/>{selected!==null&&<div className={correct?'finale-feedback correct':'finale-feedback incorrect'}><Icon name={correct?'check':'close'} /><p>{correct?question.explanation:'Bu seçim kanıtla uyuşmuyor. İlgili öğenin görevini ve sistemde hangi soruya cevap verdiğini yeniden düşün.'}</p></div>}{correct&&<button className="button button-primary finale-next" type="button" onClick={next}>{index===pack.questions.length-1?'Final skorumu göster':'Sonraki tura geç'}<Icon name="arrow-right" /></button>}</main></section>;
}
